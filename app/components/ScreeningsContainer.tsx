...
import {
  accessibilityForIndividualScreenings,
  amenities,
} from "@/Data/Filters";
...
const ScreeningsContainer = ({ screenings, showOnPage, cinemas }: Props) => {
  ...
  let filterArr: { name: string; filters: string[] }[] = [
    { name: "", filters: [""] },
  ];
  ...
  showOnPage === "film"
    ? (filterArr = [
        ...
        {
          name: "ACCESSIBILITY",
          filters: accessibilityForIndividualScreenings,
        },
  ...
  const filteredScreeningsArr = sortedScreenings
    .map((dateObject: any) => {
      const filmsOrCinema =
        showOnPage === "film" ? dateObject.cinema : dateObject.films;
      const filteredFilmsOrCinema = filmsOrCinema
        .map((filmObject: any) => {
          const filteredScreenings = filmObject.screenings.filter(
            (screening: any) => {
              const screeningAccessibility = convertCamelCaseToTitleCase(
                getTrueKeys(screening)
              );
              ...
              const isAmenityMatch =
                ...
                selectedAmenities.every((item) =>
                  screeningAmenities.includes(item)
                );
              const isAcccessibilityMatch =
                selectedAccessibility.length === 0 ||
                selectedAccessibility.every((item) =>
                  screeningAccessibility.includes(item)
                );
              const isCinemaMatch =
